...
use super::tables::{
    ...
    T_OBJECT_ATTR_CHANGES,
};
use super::value::{datetime_to_duck_timestamp, ocel_value_to_duck, to_sql_value};

/// Streaming sink for the schema. Holds one appender per table that borrow `&'con Connection` each.
///  The `events` appender is created once the wide schema is known.
/// `finalize` explicitly flushes each appender (propagating any write error) before creating indexes.
pub(crate) struct DuckDbOcelSink<'con> {
    ...
    con: &'con Connection,
    /// Event-attr schema accumulated over all `declare_event_type` calls: name -> type,
    /// widened via [`OCELAttributeType::coalesce`] when event types disagree on a name
    /// (e.g. integer + float -> float).
    ev_attr_types: HashMap<String, OCELAttributeType>,
    /// `ev_attr_types` frozen into ordered columns on the first `append_event` (all
    /// declarations have arrived by then), then extended by any column added on the fly.
    /// The type is both the column's SQL type and the value-conversion target.
...
impl<'con> DuckDbOcelSink<'con> {
    ...
    /// Add a column for an attribute never declared on an event type, so its values are
    /// stored rather than dropped. `DuckDB` refuses `ALTER TABLE` while an appender is open,
    /// hence the flush/reopen -- once per new name, not per event.
    fn add_undeclared_event_column(
        ...
        self.con.execute_batch(&add_events_column(name))?;
        ...
        self.ev_columns
            .push((name.to_string(), OCELAttributeType::String));
    ...
    /// Create the wide `events` table (sorted attribute columns for determinism) and its
    /// appender. Idempotent within a sink; called on the first `append_event` and, as a
    /// safety net for an event-less import, in `finalize`.
...
impl<'con> AppendableOCEL for DuckDbOcelSink<'con> {
    ...
    // `event_attr_meta` (for round-trip export). A name declared with different types
    // across event types widens to the type covering both (see `coalesce`).
    ...
    fn declare_object_type(&mut self, _t: OCELType) -> Result<(), Self::Error> {
        Ok(())
    }
    ...
    fn append_event(
        ...
        // `ev_attr_types` holds exactly the declared names: a name with a column but no
        // entry there came from an earlier event and still needs a meta row for this type.
        for a in &attributes {
            if !self.ev_col_index.contains_key(&a.name) {
                self.add_undeclared_event_column(&a.name, event_type)?;
            } else if !self.ev_attr_types.contains_key(&a.name) {
    ...
    fn finalize(&mut self) -> Result<(), Self::Error> {
        // Ensure `events` exists even for an event-less import, so downstream readers /
        // queries always find the table.
        self.ensure_events_created()?;
        // Explicitly flush (not just drop) so write errors (e.g. PRIMARY KEY violations)
        // surface here instead of being silently swallowed by Drop.
        if let Some(mut a) = self.events.take() {
            a.flush()?;
        }
        ...
        // Indexes are created by `run_import` as the final step, after the optional
        // file-size rewrite (which would otherwise drop them). See stream.rs.
        Ok(())
    }
}
...
mod tests {
    ...
    fn conflicting_declared_types_widen() {
        // Two event types declare `n` as integer and float: the column must widen to DOUBLE
        // (keeping it numeric), not collapse to VARCHAR.
        let con = Connection::open_in_memory().unwrap();
        create_schema(&con).unwrap();
        {
            let mut sink = DuckDbOcelSink::new(&con).unwrap();
    ...
    fn undeclared_event_attrs_get_a_column_instead_of_being_dropped() {
        ...
        let t = chrono::Utc::now().fixed_offset();
        {
            ...
            // which no event type ever declares.
            sink.declare_event_type(OCELType {
                name: "pay".into(),
                attributes: vec![OCELTypeAttribute {
                    name: "amount".into(),
            ...
            .unwrap();
            // A second event adds another new name, and re-uses `note` with a
            // non-string value: VARCHAR holds both.
            sink.append_event(
                "e2".into(),
                "ship",
                t,
        ...
        // The declared column keeps its declared type; e2 did not set it.
        let amount1: f64 = con
            .query_row(r#"SELECT "amount" FROM events WHERE id = 'e1'"#, [], |r| {
                r.get(0)
            })
    ...
    fn duplicate_event_id_surfaces_as_error() {
        ...
        sink.append_event("dup".into(), "pay", t, vec![], vec![])
            .unwrap();
        // Appending a second event with the same id violates the PRIMARY KEY on events.id.
        // Whether that surfaces here or at finalize() depends on duckdb's appender
        // buffering, but it must not be silently dropped.
